...
def delete_document(request):
    try:
        if request.method == 'DELETE':
            ...
            file = EditorFile.objects.get(id=body['document_id'])
            #   with open(file.file.path, 'w') as f:
            #   f.write('')
            obj = file.delete()
        return JsonResponse({"status": "OK", "message":"Document deleted.", "obj": obj})
    except:
        return JsonResponse({"status": "FAILED", "message":"Cannot delete the document."})
...
def delete_template(request):
    try:
        if request.method == 'DELETE':
            body = json.loads(request.body)
            file = Template.objects.get(id=body['template_id']) # get_object_or_404(Template, id=body['template_id'])
            #   with open(file.file.path, 'w') as f:
            #        f.write('')
            obj = file.delete()
            return JsonResponse({"status": "OK", "message": "Template deleted", "obj": obj})
    except:
        return JsonResponse({"status": "FAILED", "message":"Cannot delete the template."})
    ...
    def post(self, request, **kwargs):
        ...
        if company :
            ...
            if form.is_valid() :
                ...
                company.save()
                #report = save_document_entry(doc, new_data)
