...
def update_uuid_object(uuid_hash):
    ...
    response = requests.request("POST", url, headers=headers, data=payload)
    print("SAVED UUID : ", response.text)
    return response.text
...
def save_wf(wf_name, int_wf_string, ext_wf_string, user, company_id):
    ...
    response = requests.request("POST", url, headers=headers, data=payload)
    print("SAVE WORKFLOW ENTRY", response.text)
    return response.text
...
def get_template_object(template_id):
    ...
    print('Template object response', response_obj)
    res_obj = json.loads(response_obj)
    if len(res_obj["data"]) :
        return res_obj["data"]
    else:
...
def get_document_object(document_id):
    ...
    print('document object response', response_obj)
    res_obj = json.loads(response_obj)
    if len(res_obj["data"]) :
        return res_obj["data"]
    else:
