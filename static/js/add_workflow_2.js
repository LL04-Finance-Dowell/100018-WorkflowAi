...
addDocumentType.onclick = function(e){
  ...
  if(!document.getElementById('documentType-title').value){
    ...
    if( new_entry ){
        requestURL = window.location.href ;//org_id.innerHTML + '/workflow-management/add';

        requestBody = {
            title: document.getElementById('documentType-title').value,
            internal: internalWFList,
            external: externalWFList,
            org_id: org_id.innerHTML
        }
    } else {
        requestURL = window.location.href + '/update';
        requestBody = {
            id: update_ID ,
            title: document.getElementById('documentType-title').value,
            internal: internalWFList,
    ...
    fetch(createDocumentTypeRequest, {
        ...
        //  {"status": 200, "message": "workflow added.", 'obj': obj}

        if (res.status === 200){
            alert(`{res.obj} - {res.message}`);
            location.reload();
        }
        ...
        /*if( new_entry ) {
            if(selectField) {
                ...
                rlink.onclick =  (e) => {
                    ...
                    const csrftoken = document.querySelector('[name=csrfmiddlewaretoken]').value;
                    const removeDTRequest = new Request(window.location.href + '/delete', {headers: {'X-CSRFToken': csrftoken}} );
                    requestBody = {
                      id: this_id
                    }
