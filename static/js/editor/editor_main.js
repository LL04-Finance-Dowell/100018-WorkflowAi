    ...
    const rejectDocumentForm = (event) => {
        ...
        submitInputButton.onclick = (event) => {
            ...
            const requestURL = `/reject-document-request`;
...
if(prevSibling == null ){
    ...
    const sendSaveDocumentRequest = (data) => {
        ...
        const requestURL = window.location.href;    // isTemplate ? '/editor/api/save-template/' :'/editor/api/save-file/';
        ...
        // }
        const documentID = JSON.parse(document.getElementById('documentID').textContent);
        const user = JSON.parse(document.getElementById('curr_user').textContent);
        // const documentName = JSON.parse(document.getElementById('documentName').textContent);
        let alertBox = document.getElementById('showAlert');
...
if(requestURL != "error"){
        fetch(saveFileRequest, {
            ...
            if (fileObject.status === 200){
                let alertSuccessMessage = '<button type="button" class="close" data-dismiss="alert">&times;</button><strong>Success! </strong>'+ documentName + ' file processed succesfully.';
                ...
                let alertSuccessMessage = '<button type="button" class="close" data-dismiss="alert">&times;</button><strong>Error! </strong>'+ documentName + ' save error.';
                // alertSuc.append(butCl);
                alertSuc.innerHTML =alertSuccessMessage;
    ...
    function addWorkflow() {
        ...
        const csrftoken = document.querySelector('[name=csrfmiddlewaretoken]').value;
        const requestURL =  `/add-to-wf`;                   //          window.location.hostname + https://100084.pythonanywhere.com/document/add-to-wf
...
currentP.parentNode.insertBefore(newPageHolder, nextP);
    ...
    function renderFile(data){
        ...
        for( let pageContent of content ){
            ...
            addPage.id= "addPageHereId";
            addPage.textContent  = "Add Page";

            addPage.className = "btn btn-primary btn-sm";
            addPage.style.marginTop  ="1%";
            addPage.onclick=function(event){addPageHere3(event)}

            deletePage.className = "btn btn-primary btn-sm";
            deletePage.style.marginTop  ="1%";
            deletePage.style.marginLeft = "1%";
            deletePage.id= "deletePageHereId";
            deletePage.textContent  = "Delete Page";
            deletePage.onclick=function(event){deletePageHere3(event)}
...
if( item.type === "TEXT_INPUT" ){
            ...
            newPageHolder.append(newPage);
            if(isTemplate){
                         newPageHolder.append(addPage);
                         newPageHolder.append(deletePage);
                    }
            pageContainer.append(newPageHolder);
            }
