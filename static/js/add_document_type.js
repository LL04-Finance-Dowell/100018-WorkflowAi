...
const createRowInTable = (item, list, table) => {
    ...
    rm_link.onclick = ( event ) => {
        ...
        let found =  list.find(item => (item.name === event.target.parentNode.parentNode.childNodes[1].innerHTML) && (item.emailID === event.target.parentNode.parentNode.childNodes[2].innerHTML) );
...
function submit_step(list, type=""){
    ...
    document.getElementById(type + 'emailID').style.borderColor = 'initial';

    const stepObj = {
        name: document.getElementById( type + 'StepName').value,
        emailID: document.getElementById(type + 'emailID').value
    }

    document.getElementById( type + 'StepName').value = ''
    document.getElementById(type + 'emailID').value = ''

    if((stepObj.name === '') || (stepObj.emailID === '')){
        ...
        document.getElementById(type + 'emailID').style.borderColor = '#d00';

    } else {

        let found =  list.find(item => (item.name === stepObj.name) && (item.emailID === stepObj.emailID)
        );
            ...
            if(selectField) {
                ...
                if (documentTypeObject.internal_work_flow) {
                    ...
                    for(step of documentTypeObject.internal_work_flow.steps){
                        con += step.name + ' - ' + step.emailID + ',';

                        const li = document.createElement('li');
                        li.innerHTML = step.name + ',' + step.emailID;
